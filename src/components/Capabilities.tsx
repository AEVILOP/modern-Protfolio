...
export default function Capabilities() {
  ...
  return (
    <section className="relative z-20 bg-[#121212] py-32 px-6 md:px-12 border-t border-white/5">
      <div className="max-w-5xl mx-auto flex flex-col items-center">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
          {capabilities.map((item, idx) => (
            <div 
              key={idx}
              className="group p-8 md:p-10 rounded-3xl bg-[#171717] border border-white/5 hover:border-white/10 transition-colors duration-300"
            >
              <div className="w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center mb-8 group-hover:bg-white/10 transition-colors duration-300">
                {item.icon}
              </div>
